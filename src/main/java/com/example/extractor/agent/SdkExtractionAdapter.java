 ...
 * Falls back to direct invocation if SDK has limitations (e.g., text-only
 * support).
 */
public class SdkExtractionAdapter implements ExtractionAdapter {
    ...
    public SdkExtractionAdapter(ComponentClient componentClient, String providerName,
            DirectExtractionAdapter fallbackAdapter, String agentId) {
        this.agentClient = componentClient != null ? new ExtractionAgentClient(componentClient, agentId) : null;
    ...
    public CompletionStage<String> extract(String prompt, List<Path> files) {
        ...
        try {
            return agentClient.extract(prompt, files)
                    .exceptionally(ex -> {
                        // If SDK fails (e.g., AgentResult creation issues), fall back to direct
                        // invocation
                        System.err.println(
                                "SDK Agent extraction failed, falling back to direct invocation: " + ex.getMessage());
                        return fallbackAdapter.extract(prompt, files)
                                .toCompletableFuture()
